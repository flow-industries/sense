...
} from "@extensions/factories/SenseFactory.sol";
...
contract SenseFactoryTest is Test, BaseDeployments {
    Namespace namespace;
    ...
    function setUp() public override {
        super.setUp();
        ownerAccount = senseFactory.deployAccount({
            metadataURI: "uri://any",
            owner: address(this),
    ...
    function testCreateAccountWithUsernameFree() public {
        ...
        senseFactory.createAccountWithUsernameFree({
    ...
    function testDeployGroup() public {
        address group = abi.decode(
            IAccount(payable(ownerAccount)).executeTransaction(
                address(senseFactory),
                0,
                abi.encodeWithSelector(
                    SenseFactory.deployGroup.selector,
                    "uri://group",
                    ownerAccount,
                    _emptyAddressArray(),
    ...
    function testCreateGroupWithFeed() public {
        (address group, address feed) = abi.decode(
            IAccount(payable(ownerAccount)).executeTransaction(
                ...
                0,
                abi.encodeCall(
                    SenseFactory.createGroupWithFeed,
                    (
                        ownerAccount,
                        _emptyAddressArray(),
                        GroupWithFeed_GroupParams({
    ...
    function _assertFeedSetup_createGroupWithFeed(address feed, address /* group */ ) internal view {
        ...
        KeyValue[] memory factoryRules = senseFactory.getRules();
        for (uint256 i = 0; i < factoryRules.length; i++) {
            if (factoryRules[i].key == keccak256("sense.address.AccountBlockingRule")) {
                accountBlockingRuleAddress = abi.decode(factoryRules[i].value, (address));
            } else if (factoryRules[i].key == keccak256("sense.address.GroupGatedFeedRule")) {
    ...
    function _assertProcessAdditionGroupRules(address group) internal view {
        ...
        KeyValue[] memory factoryRules = senseFactory.getRules();
        for (uint256 i = 0; i < factoryRules.length; i++) {
            if (factoryRules[i].key == keccak256("sense.address.AdditionRemovalPidGroupRule")) {
    ...
    function _assertProcessJoiningGroupRules(address group) internal view {
        ...
        KeyValue[] memory factoryRules = senseFactory.getRules();
        for (uint256 i = 0; i < factoryRules.length; i++) {
            if (factoryRules[i].key == keccak256("sense.address.BanMemberGroupRule")) {
