...
abstract contract SenseERC721 is IERC721Metadata, ERC165 {
    using AddressUpgradeable for address;

    event Sense_ERC721_TokenURIProviderSet(address indexed tokenURIProvider);
    event Sense_ERC721_Initialized(string name, string symbol);
    ...
    /// @custom:keccak sense.storage.ERC721
    bytes32 constant STORAGE__ERC721 = 0x15a624ab72d0b5a87b0e548a0c7afd7baef646d0ba4c6b1f8fd786e8c9f2eb89;
    ...
    function _initialize(string memory nftName, string memory nftSymbol, ITokenURIProvider tokenURIProvider) internal {
        ...
        emit Sense_ERC721_Initialized(nftName, nftSymbol);
        emit Sense_ERC721_TokenURIProviderSet(address(tokenURIProvider));
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = SenseERC721.ownerOf(tokenId);
    ...
    function _ownerOf(uint256 tokenId) internal view virtual returns (address) {
        return $erc721Storage().owners[tokenId];
    }
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        owner = SenseERC721.ownerOf(tokenId);
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(SenseERC721.ownerOf(tokenId) == from, Errors.InvalidParameter());
        require(to != address(0), Errors.InvalidParameter());
        ...
        unchecked {
            // `balances[from]` cannot overflow for the same reason as described in `_burn`:
            // `from`'s balance is the number of token held, which is at least one before the current
            // transfer.
            // `balances[to]` could overflow in the conditions described in `_mint`. That would require
            // all 2**256 token ids to be minted, which in practice is impossible.
            $erc721Storage().balances[from] -= 1;
            $erc721Storage().balances[to] += 1;
        }
        $erc721Storage().owners[tokenId] = to;
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(SenseERC721.ownerOf(tokenId), to, tokenId);
    }
