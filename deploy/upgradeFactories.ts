...
import {
  ...
  async function deploy() {
    ...
    for (const factory of factoriesToUpgrade) {
      const upgradedFactory = await deployImplAndUpgradeTransparentProxy(proxyOwnerWallet, factory);
    ...
    for (const upgradedFactory of upgradedFactories) {
      ...
      const artifactBytecodeHash = await getArtifactBytecodeHash(upgradedFactory.name!);
