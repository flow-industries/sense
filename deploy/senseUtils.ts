...
export async function deploySenseContract(
...
export async function deploySenseContractAsProxy(
  contractToDeploy: ContractInfo,
  proxyOwner: string,
  initializerCalldata?: string
...
export async function deploySenseContractWithCreate2(
  contractToDeploy: ContractInfo,
  ...
  const onchainOwner = await senseCreate2.owner();
  if (senseCreate2OwnerWallet.address !== onchainOwner) {
    throw new Error(`SenseCreate2 owner mismatch: Assume ${senseCreate2OwnerWallet.address} !== Onchain ${onchainOwner}`);
  }
  ...
  ////////// Deploying with SenseCreate2

  const proxyArtifact = await hre.artifacts.readArtifact("TransparentUpgradeableProxy");
  const proxyBytecodeHash = calculateBytecodeHash(proxyArtifact.bytecode);

  if (!addressBook[contractToDeploy.contractName]) {
    const preSalt = 'sense.contract.' + contractToDeploy.contractName;
    ...
    console.log(`Deploying ${contractToDeploy.contractName} proxy through SenseCreate2`);
...
export function mapContractNameToEnvVarName(contractName: string): string {
  if (contractName === 'SenseGlobalFeed') {
    return 'GLOBAL_FEED';
  } else if (contractName === 'SenseGlobalGraph') {
    return 'GLOBAL_GRAPH';
  } else if (contractName === 'SenseGlobalNamespace') {
    return 'SENSE_NAMESPACE';
  } else {
    return contractName
...
export function generateEnvFile() {
  ...
  if (primitives.length > 0) {
    output += '# SENSE GLOBAL PRIMITIVES\n';
