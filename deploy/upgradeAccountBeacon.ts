...
async function deploy() {
  ...
  for (const upgradedBeacon of upgradedBeacons) {
    const implementationOnchain = await getBeaconImplementationAddress(
      upgradedBeacon.beaconInfo.address!
    );
    ...
    if (bytecodeHash !== artifactBytecodeHash) {
      throw new Error(
        `${upgradedBeacon.name} bytecode hash mismatch: ${bytecodeHash} !== ${artifactBytecodeHash}`
      );
    } else {
      console.log(
        `\x1b[32m${upgradedBeacon.name} onchain bytecode hash verified: ${bytecodeHash} and matches artifact bytecode hash\x1b[0m`
      );
    }
  }
}
