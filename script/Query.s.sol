...
contract Query is Script {
    ...
    function _generatePostId(address author, uint256 authorPostSequentialId) internal pure returns (uint256) {
        return uint256(keccak256(abi.encode("evm:", 271, SENSE_GLOBAL_FEED, author, authorPostSequentialId)));
    }

    function run() external view {
        console.log("ChainID: ", block.chainid);
        address author = address(0x2aa01F5cDF6403B3b53826e7798790069185bE2F);
        uint256 postCount = IFeed(SENSE_GLOBAL_FEED).getPostCount(author);
        console.log("Post Count: ", postCount);
        for (uint256 i = 0; i < postCount + 5; i++) {
            uint256 postId = _generatePostId(author, i);
            console.log("Sequence: ", i);
            console.log("Post ID: ", postId);
            bool exists = IFeed(SENSE_GLOBAL_FEED).postExists(postId);
            if (exists) {
                console.log("Exists");
                Post memory post = IFeed(SENSE_GLOBAL_FEED).getPost(postId);
